...
func (r *MovieGenreRepository) GetGenresOfMovie(ctx context.Context, movieID int) ([]genres.Genre, error) {
	sqlQuery := `
	SELECT 
	genres.id,
	genres.name
	...
	var movieGenres []genres.Genre

	for rows.Next() {
		var movieGenre genres.Genre
		err := rows.Scan(
			&movieGenre.ID,
			&movieGenre.Name,
		)
