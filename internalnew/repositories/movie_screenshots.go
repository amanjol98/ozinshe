...
func (r *MovieScreenshotsRepository) GetScreenshots(ctx context.Context, movieID int) ([]MovieScreenshot, error) {
	sqlQuery := `
	SELECT
	ms.id,
	ms.image_url
	...
	var movieScreenshots []MovieScreenshot

	for rows.Next() {
		var movieScreenshot MovieScreenshot
